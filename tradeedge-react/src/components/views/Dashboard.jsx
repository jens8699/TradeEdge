    ...
    <div className="jm-view">
      ...
      >
        ...
        {todayTrades.length === 0
          ...
          : <p className="jm-hero-meta">{today.count} trade{today.count === 1 ? '' : 's'} · {today.wins}W / {today.losses}L  win rate</p>
        }
      </div>

      {/* Quick stats row */}
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap:'10px', marginBottom:'14px' }}>
        <QuickCard label="This week" val={fmt(week.totalPnl)} sub={`${week.count} trades - ${week.winRate.toFixed(0)}% WR`} positive={week.totalPnl >= 0} onClick={() => setActiveTab('stats')} />
        <QuickCard label="This month" val={fmt(month.totalPnl)} sub={`${month.count} trades - ${month.winRate.toFixed(0)}% WR`} positive={month.totalPnl >= 0} onClick={() => setActiveTab('stats')} />
        <QuickCard label="Win streak" val={streak >= 1 ? `${streak} 🔥` : '—'} sub={streak >= 1 ? 'consecutive wins' : 'no active streak'} positive={streak >= 3} neutral={streak < 1} />
      </div>
      ...
      <div className="jm-card">
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'12px' }}>
          ...
          <button onClick={() => setActiveTab('history')} style={{ fontSize:'12px', color:'#E8724A', background:'none', border:'none', cursor:'pointer', padding:0 }}>View all →</button>
        </div>
        {recentTrades.length === 0 ? (
          <div style={{ textAlign:'center', padding:'24px 0' }}>
            <p style={{ color:'var(--c-text-2)', fontSize:'13px', margin:'0 0 12px' }}>No trades logged yet.</p>
            <button className="jm-btn-primary" onClick={() => setActiveTab('entry')} style={{ fontSize:'13px', padding:'8px 20px' }}>Log your first trade</button>
          </div>
        ) : (
              ...
              <div key={t.id} className="dash-trade-row">
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:'8px' }}>
                    <span style={{ fontWeight:600, fontSize:'13px', color:'var(--c-text)' }}>{t.ticker || '—'}</span>
                    {t.direction && <span style={{ fontSize:'10px', fontWeight:600, padding:'2px 6px', borderRadius:'4px', background: t.direction === 'Long' ? 'rgba(93,202,165,0.15)' : 'rgba(226,75,74,0.15)', color: t.direction === 'Long' ? '#5DCAA5' : '#E8724A', textTransform:'uppercase', letterSpacing:'0.5px' }}>{t.direction}</span>}
                    {t.setup && <span style={{ fontSize:'11px', color:'var(--c-text-2)' }}>{t.setup}</span>}
                  </div>
                  <p style={{ margin:'2px 0 0', fontSize:'11px', color:'var(--c-text-2)' }}>{t.date}</p>
                </div>
                <span style={{ fontWeight:700, fontSize:'14px', fontVariantNumeric:'tabular-nums', color: t.pnl > 0 ? '#5DCAA5' : t.pnl < 0 ? '#E24B4A' : 'var(--c-text-2)' }}>{t.pnl > 0 ? '+' : ''}{fmt(t.pnl)}</span>
              </div>
            ))}
          </div>
        )}
      ...
      <div style={{ display:'flex', gap:'10px', marginTop:'14px' }}>
        <button className="jm-btn-primary" onClick={() => setActiveTab('entry')} style={{ flex:1, justifyContent:'center' }}>✦ Log Trade</button>
        <button onClick={() => setActiveTab('stats')} style={{ flex:1, padding:'10px', borderRadius:'10px', border:'1px solid var(--c-border)', background:'var(--c-surface)', color:'var(--c-text)', fontSize:'13px', fontWeight:500, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:'6px' }}>◈ View Stats</button>
      </div>
    </div>
  );
}
...
function QuickCard({ label, val, sub, positive, neutral, onClick }) {
  return (
    <div className="jm-metric" onClick={onClick} style={{ cursor: onClick ? 'pointer' : 'default' }}>
      <p className="jm-metric-label">{label}</p>
      <p className="jm-metric-val" style={{ color: neutral ? 'var(--c-text-2)' : positive ? '#5DCAA5' : '#E24B4A', fontSize:'15px' }}>{val}</p>
