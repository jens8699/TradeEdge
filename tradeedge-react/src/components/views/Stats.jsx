...
function getSetupStats(list) {
  ...
  list.filter(t => t.setup).forEach(t => {
    ...
    map[t.setup].pnl += t.pnl; map[t.setup].count++;
    if (t.pnl > 0) map[t.setup].wins++; else if (t.pnl < 0) map[t.setup].losses++;
  });
...
export default function Stats() {
  ...
  const heroRef = useRef(null); const chartRef = useRef(null);
  const wlRef = useRef(null); const dowRef = useRef(null);
  const pnlChart = useRef(null); const wlChart = useRef(null); const dowChart = useRef(null);
  ...
  useEffect(() => { if (heroRef.current) animateCount(heroRef.current, s.totalPnl); }, [s.totalPnl, period]);
  ...
  useEffect(() => {
    ...
    import('chart.js').then(({ Chart, registerables }) => {
      ...
      if (ctx && sorted.length) {
        ...
        const lc = cum >= 0 ? '#E8724A' : '#E24B4A';
        const fc = cum >= 0 ? 'rgba(232,114,74,0.15)' : 'rgba(226,75,74,0.12)';
        pnlChart.current = new Chart(ctx, { type: 'line', data: { labels, datasets: [{ data, borderColor: lc, backgroundColor: fc, fill: true, tension: 0.35, pointRadius: sorted.length<20?4:0, pointBackgroundColor: lc, pointBorderColor: '#17150F', pointBorderWidth: 2, borderWidth: 2.5 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false }, tooltip: { backgroundColor: '#0E0C08', titleColor: '#F5F3ED', bodyColor: '#E8E6E1', borderColor: '#2A2720', borderWidth: 1, padding: 10, cornerRadius: 8, callbacks: { label: c => (c.raw<0?'-$':'$')+Math.abs(c.raw).toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2}) } } }, scales: { y: { ticks: { color: '#8B8882', callback: v => (v<0?'-$':'$')+Math.abs(v) }, grid: { color: 'rgba(255,255,255,0.04)' }, border: { display: false } }, x: { ticks: { color: '#8B8882', maxTicksLimit: 8 }, grid: { display: false }, border: { display: false } } } } });
      }
      if (wlChart.current) { wlChart.current.destroy(); wlChart.current = null; }
      const wlCtx = wlRef.current;
      if (wlCtx) {
        const be = s.count - s.wins - s.losses;
        wlChart.current = new Chart(wlCtx, { type: 'bar', data: { labels: ['Wins','Losses','B/E'], datasets: [{ data: [s.wins,s.losses,be], backgroundColor: ['rgba(93,202,165,0.75)','rgba(226,75,74,0.75)','rgba(139,136,130,0.4)'], borderRadius: 8, borderWidth: 0 }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend:{display:false}, tooltip:{backgroundColor:'#0E0C08',titleColor:'#F5F3ED',bodyColor:'#E8E6E1',borderColor:'#2A2720',borderWidth:1,padding:10,cornerRadius:8} }, scales: { y:{ticks:{color:'#8B8882'},grid:{color:'rgba(255,255,255,0.04)'},border:{display:false}}, x:{ticks:{color:'#E8E6E1'},grid:{display:false},border:{display:false}} } } });
      }
  ...
  return (
    <div className="jm-view">
      ...
      <div className="jm-hero" style={s.totalPnl<0?{background:'radial-gradient(ellipse at top right,rgba(226,75,74,0.22) 0%,rgba(226,75,74,0.04) 50%,#1E1C16 100%)',borderColor:'rgba(226,75,74,0.4)'}:{}}>
        ...
        {list.length===0 ? <p className="jm-hero-meta">No trades in this period</p> : <p className="jm-hero-meta">{s.count} trade{s.count!==1?'s':''} ÃÂ· {s.wins}W / {s.losses}L{consistencyInfo?' ÃÂ· ':''}{consistencyInfo&&<span style={{color:consistencyInfo.color,fontWeight:600}}>{consistency} consistency score</span>}</p>}
      </div>
      {list.length===0 ? (
        ...
        <div className="stats-rings-row" style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'10px',marginBottom:'10px'}}>
          ...
          <MetricRing label="Profit Factor" value={isFinite(s.pf)?s.pf.toFixed(2):'Ã¢ÂÂ'} pct={Math.min((isFinite(s.pf)?s.pf:3)/3*100,100)} color="#5DCAA5" foot={s.pf>=1.5?'Ã¢ÂÂ Good edge':s.pf>=1?'Breakeven zone':'Ã¢ÂÂ  Below B/E'} />
          <MetricRing label="Avg R:R" value={s.rr.toFixed(2)} pct={Math.min(s.rr/3*100,100)} color="#85B7EB" foot={s.rr>=1.5?'Ã¢ÂÂ Healthy R:R':s.rr>=1?'Needs improvement':'Ã¢ÂÂ  Risk > Reward'} />
        </div>
        ...
        <div className="jm-card stats-streak-card" style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:'12px',marginBottom:'10px',padding:'16px 20px'}}>
          <StreakCol label="Current streak" value={streak.current} type={streak.type} icon={streak.type==='win'?'Ã°ÂÂÂ¥':streak.type==='loss'?'Ã°ÂÂ§Â':'Ã¢ÂÂ'} sub={streak.current===0?'Start trading':streak.type==='win'?streak.current+' green in a row':streak.current+' red in a row'} />
          <StreakCol label="Longest win run" value={streak.longest} type="win" icon="Ã¢Â­Â" sub="consecutive wins" />
          <StreakCol label="Longest loss run" value={streak.longestLoss} type="loss" icon="Ã¢ÂÂÃ¯Â¸Â" sub="consecutive losses" />
        </div>
        {setups.length>0&&(
                ...
                <div key={ss.name} style={{display:'flex',flexDirection:'column',gap:'4px'}}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'baseline'}}>
                    ...
                    <div style={{display:'flex',gap:'12px',alignItems:'center'}}>
                      <span style={{fontSize:'11px',color:'#8B8882'}}>{ss.count} trades ÃÂ· {ss.wr}% WR</span>
                      <span style={{fontSize:'13px',fontWeight:700,color:ss.pnl>=0?'#5DCAA5':'#F09595'}}>{fmt(ss.pnl)}</span>
                    </div>
                  </div>
                  <div style={{height:'5px',borderRadius:'3px',background:'rgba(255,255,255,0.06)',overflow:'hidden'}}>
                    <div style={{height:'100%',borderRadius:'3px',width:Math.abs(ss.pnl)/maxSetupPnl*100+'%',background:ss.pnl>=0?'linear-gradient(90deg,#E8724A,#F0A67A)':'linear-gradient(90deg,#E24B4A,#F09595)',transition:'width 0.6s ease'}} />
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="stats-charts-row" style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px',marginBottom:'10px'}}>
          <div className="jm-card"><h2 className="jm-card-title" style={{marginBottom:'12px'}}>Cumulative P/L</h2><div style={{position:'relative',height:'200px'}}><canvas ref={chartRef}/></div></div>
          <div className="jm-card"><h2 className="jm-card-title" style={{marginBottom:'12px'}}>Win / Loss Split</h2><div style={{position:'relative',height:'200px'}}><canvas ref={wlRef}/></div></div>
        </div>
        {dowStats.length>1&&(
          <div className="jm-card" style={{marginBottom:'10px'}}>
          ...
          <div className="stats-charts-row" style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px',marginBottom:'10px'}}>
            {topWins.length>0&&(
              <div className="jm-card">
                <h2 className="jm-card-title" style={{marginBottom:'12px',color:'#5DCAA5'}}>Ã°ÂÂÂ Top Wins</h2>
            ...
            {topLosses.length>0&&(
              <div className="jm-card">
                <h2 className="jm-card-title" style={{marginBottom:'12px',color:'#F09595'}}>Ã°ÂÂÂ Biggest Lessons</h2>
